...
CREATE UNIQUE INDEX
  IF NOT EXISTS idx_orders_customer_date ON orders(customer_id, date(created_at));

CREATE INDEX
  IF NOT EXISTS idx_profiles_name ON "user profiles"([full name]);

-- WITHOUT ROWID table
CREATE TABLE
  settings_lookup(key TEXT PRIMARY KEY, value TEXT, category TEXT) WITHOUT ROWID;
...
SELECT
  ...
  monthly_total,
  monthly_rank,
  monthly_total - prev_month_total as growth
FROM
  customer_rankings;
...
SELECT
  ...
  o.customer_id,
  o.total,
  up."full name" as customer_name,
  o.status
FROM
  orders o
  JOIN "user profiles" up ON o.customer_id = up.user_id
WHERE
  o.id = ?
  ...
  AND o.status = $status
  AND o.created_at > ?2
  AND up.`profile_data` IS NOT NULL;

-- JSON operations (SQLite 3.38+)
SELECT
  customer_id,
  metadata ->> 'name' as customer_name,
  metadata -> 'preferences' ->> 'theme' as theme,
  metadata -> 'settings' as all_settings
FROM
  orders
WHERE
  metadata ->> 'status' = 'premium'
  AND metadata -> 'preferences' ->> 'notifications' = 'enabled';
...
SELECT
  customer_id || ': ' || status as customer_status,
  'Order #' || CAST(id AS TEXT) || ' - $' || CAST(total AS TEXT) as order_summary
FROM
  orders
WHERE
  total > 100.0;
...
INSERT INTO
  file_storage (filename, content_type, data)
VALUES
  (
    'document.pdf',
    'application/pdf',
...
('image.png', 'image/png', X'89504E470D0A1A0A');

-- UPSERT with ON CONFLICT
INSERT INTO
  customer_stats (customer_id, total_orders, total_spent)
VALUES
  (?1, ?2, ?3)
ON CONFLICT
(customer_id)
DO UPDATE
SET
  total_orders = excluded.total_orders + customer_stats.total_orders,
  total_spent = excluded.total_spent + customer_stats.total_spent,
  updated_at = datetime('now');

-- INSERT OR variations
INSERT OR IGNORE
  INTO temp_cache (key, value)
VALUES
  ('session:123', 'user_data');

INSERT OR REPLACE
  INTO user_sessions (user_id, session_token, expires_at)
VALUES
  (42, 'abc123def456', datetime('now', '+24 hours'));
...
WHERE
  ...
  AND total IS NOT DISTINCT FROM 0.0
...
FROM
  files
WHERE
  filename GLOB '*.{jpg,png,gif}'
  OR path LIKE '%/uploads/%'
  AND filename NOT GLOB 'temp*';
...
WITH
  ...
  sales_by_category AS (
    SELECT
      ...
      RANK() OVER (
        PARTITION BY ch.depth
        ORDER BY
          SUM(oi.quantity * oi.unit_price) DESC
      ) as revenue_rank,
      LAG(SUM(oi.quantity * oi.unit_price), 1) OVER (
        PARTITION BY ch.depth
        ORDER BY
          SUM(oi.quantity * oi.unit_price) DESC
      ) as prev_category_revenue
    FROM
      category_hierarchy ch
      LEFT JOIN products p ON p.category_id = ch.id
      LEFT JOIN order_items oi ON oi.product_id = p.id
    WHERE
      oi.created_at >= date('now', '-3 months')
...
SELECT
  ...
  avg_order_value,
  revenue_rank,
  COALESCE(total_revenue - prev_category_revenue, 0) as revenue_gap
FROM
  sales_by_category
WHERE
  total_revenue > 1000
...
SELECT
  ...
  LEAD(total) OVER (
    PARTITION BY customer_id
    ORDER BY
      order_date RANGE BETWEEN CURRENT ROW
...
DROP INDEX
  IF EXISTS idx_orders_customer_date;
