...
CREATE TRIGGER
  validate_product_before_insert
  BEFORE
INSERT
  ON products
  FOR EACH ROW BEGIN
    SELECT
      CASE
        WHEN NEW.price <= 0 THEN RAISE(ABORT, 'Price must be positive')
    ...
    SELECT
      CASE
        ...
        AND OLD.status != 'cancelled' THEN (
          INSERT INTO
            cancelled_orders (order_id, reason, cancelled_at)
          VALUES
