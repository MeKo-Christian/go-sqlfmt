...
ORDER BY
  ...
  created_at DESC
LIMIT
  20, 10 -- MySQL-style offset, limit
FOR UPDATE
;
...
WITH RECURSIVE
  department_tree AS (
    ...
    WHERE
      dt.level < 5 -- Prevent infinite recursion
  ),
  employee_stats AS (
    SELECT
      e.id,
      ...
      JSON_ARRAYAGG(e.skill) OVER (
        PARTITION BY e.department_id
        ORDER BY
          e.salary DESC ROWS BETWEEN 2 PRECEDING
          AND 2 FOLLOWING
      ) as peer_skills
    FROM
      employees e
...
CREATE UNIQUE INDEX
  uk_product_name ON products (name) USING BTREE;

CREATE INDEX
  idx_price_range ON products (base_price, discounted_price) USING BTREE;

CREATE SPATIAL INDEX
  sp_location ON venues (coordinates) USING RTREE;
