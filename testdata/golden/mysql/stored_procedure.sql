...
CREATE PROCEDURE
  CheckInventory(IN product_id INT, OUT status VARCHAR(20)) BEGIN
    DECLARE stock INT;
...
CREATE PROCEDURE
  GenerateSequence(IN max_value INT) BEGIN
...
DROP TEMPORARY TABLE sequence_table;

END;

-- Procedure with cursor and handler
CREATE PROCEDURE
  CalculateTotalSales() BEGIN
...
CREATE PROCEDURE
  ProcessOrders(IN order_date DATE) BEGIN
