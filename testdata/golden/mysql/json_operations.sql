...
FROM
  product_catalog
WHERE
  JSON_EXTRACT(features, '$[0].enabled') = true;
...
FROM
  analytics_events
WHERE
  event_data->'$.user'->>'$.role' IN ('admin', 'moderator')
...
SELECT
  ...
  AVG(
    CAST(
      metadata->'$.rating'->>'$.average' AS DECIMAL(3, 2)
    )
  ) as avg_rating
FROM
  products
WHERE
  metadata->'$.featured'->>'$.status' = 'active'
GROUP BY
  category
HAVING
  COUNT(*) >= 3;
...
SELECT
  ...
  LAG(JSON_LENGTH(session_data->'$.actions'), 1) OVER (
    ...
    ORDER BY
      session_data->'$.timestamp'->>'$.start'
  ) as prev_action_count,
  SUM(JSON_LENGTH(session_data->'$.actions')) OVER (
    PARTITION BY user_id
    ORDER BY
      session_data->'$.timestamp'->>'$.start' ROWS BETWEEN UNBOUNDED PRECEDING
      AND CURRENT ROW
  ) as cumulative_actions
FROM
  user_sessions
WHERE
  session_data->'$.timestamp'->>'$.start' >= CURDATE()
ORDER BY
  user_id,
  session_data->'$.timestamp'->>'$.start';