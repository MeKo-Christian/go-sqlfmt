...
VALUES (1, 'john_doe', 'john@example.com', 'active')
ON DUPLICATE KEY UPDATE 
    email = VALUES(email),
    updated_at = NOW();

-- Multiple value rows with ON DUPLICATE KEY UPDATE
INSERT INTO product_inventory (product_id, location_id, quantity, reserved) 
VALUES 
    (100, 1, 50, 0),
    (101, 1, 75, 5),
    (102, 1, 25, 2)
ON DUPLICATE KEY UPDATE 
    ...
    reserved = GREATEST(reserved, VALUES(reserved)),
...
ON DUPLICATE KEY UPDATE 
    ...
    last_login = GREATEST(last_login, VALUES(last_login)),
    ...
    streak_days = CASE 
        WHEN DATEDIFF(VALUES(last_login), last_login) = 1 
        THEN streak_days + 1
        WHEN DATEDIFF(VALUES(last_login), last_login) > 1 
        THEN 1
        ELSE streak_days
...
INSERT IGNORE INTO unique_tokens (token_hash, user_id, expires_at, token_type)
VALUES (SHA2('random_token_123', 256), 42, DATE_ADD(NOW(), INTERVAL 1 HOUR), 'session');
...
REPLACE INTO cache_entries (cache_key, cache_value, expires_at, cache_tags)
VALUES (
    'user:profile:123', 
...
REPLACE INTO daily_summaries (summary_date, user_id, page_views, session_duration, actions_taken)
VALUES 
    (CURDATE(), 100, 45, 1800, JSON_ARRAY('login', 'view_dashboard', 'create_report')),
    (CURDATE(), 101, 23, 900, JSON_ARRAY('login', 'view_profile')),
    (CURDATE(), 102, 67, 3600, JSON_ARRAY('login', 'view_dashboard', 'edit_settings', 'logout'));

-- Complex REPLACE with JSON and generated values
REPLACE INTO user_activity_log (
    user_id,
    activity_date,
    activity_data,
    activity_summary,
    ...
    JSON_OBJECT(
        ...
        'actions', JSON_ARRAY('login', 'view_reports', 'export_data')
    ),
    'User performed high-privilege actions',
    CASE 
        WHEN JSON_CONTAINS(
            JSON_OBJECT('actions', JSON_ARRAY('export_data', 'admin_panel')),
            JSON_EXTRACT(VALUES(activity_data), '$.actions[*]')
        ) THEN 'HIGH'
        ELSE 'NORMAL'
    END
);
...
ON DUPLICATE KEY UPDATE
    session_data = JSON_MERGE_PATCH(
        ...
        JSON_OBJECT(
            'last_seen', NOW(),
            'page_count', COALESCE(JSON_EXTRACT(session_data, '$.page_count'), 0) + 1
        )
    ),
...
ON DUPLICATE KEY UPDATE
    operation = CONCAT(operation, ',', VALUES(operation)),
    new_values = JSON_MERGE_PATCH(new_values, VALUES(new_values)),
    changed_at = VALUES(changed_at),
    change_count = COALESCE(change_count, 0) + 1;

-- Upsert with complex WHERE-like logic in UPDATE clause
INSERT INTO price_history (product_id, price, effective_date, currency, price_source)
VALUES (?, ?, CURDATE(), 'USD', 'api')
ON DUPLICATE KEY UPDATE
    price = CASE 
        WHEN VALUES(price) != price AND ABS(VALUES(price) - price) / price > 0.1 
        THEN VALUES(price)  -- Only update if price change > 10%
        ELSE price
    END,
    last_updated = CASE 
