...
CREATE FULLTEXT INDEX ft_post_content ON posts (title, content);
CREATE SPATIAL INDEX sp_location ON venues (coordinates) USING HASH;

-- Multi-column indexes
CREATE INDEX idx_user_status_created ON users (status, created_at, updated_at) USING BTREE;
CREATE UNIQUE INDEX uk_product_sku_store ON products (sku, store_id, is_active) USING BTREE;

-- ALTER TABLE with options  
ALTER TABLE users ADD COLUMN status VARCHAR(20) DEFAULT 'active', ALGORITHM=INSTANT, LOCK=NONE;
ALTER TABLE products MODIFY COLUMN price DECIMAL(10,2) NOT NULL, ALGORITHM=INPLACE, LOCK=SHARED;
ALTER TABLE orders ADD CONSTRAINT chk_total CHECK (total > 0), ADD INDEX idx_order_date (created_at), ALGORITHM=COPY, LOCK=EXCLUSIVE;
...
CREATE FULLTEXT INDEX ft_names ON users (first_name, last_name);
