...
CREATE TABLE user_sessions WITHOUT ROWID (
...
CREATE INDEX IF NOT EXISTS idx_inventory_status ON inventory(needs_reorder, quantity);
CREATE INDEX IF NOT EXISTS idx_sessions_user_active ON user_sessions(user_id) WHERE NOT is_expired;
...
CREATE TABLE user_preferences (
    ...
    notifications_enabled GENERATED ALWAYS AS (json_extract(settings, '$.notifications.enabled')) VIRTUAL,
    language GENERATED ALWAYS AS (COALESCE(json_extract(settings, '$.language'), 'en')) STORED
);

-- CREATE INDEX on generated columns
CREATE INDEX IF NOT EXISTS idx_preferences_theme ON user_preferences(theme);
CREATE INDEX IF NOT EXISTS idx_preferences_language ON user_preferences(language);

-- Complex table combining all features
CREATE TABLE audit_log STRICT (
    id INTEGER PRIMARY KEY,
    ...
    has_sensitive_data GENERATED ALWAYS AS (
        json_extract(new_values, '$.password') IS NOT NULL OR
        json_extract(new_values, '$.ssn') IS NOT NULL OR
        json_extract(old_values, '$.password') IS NOT NULL OR
        json_extract(old_values, '$.ssn') IS NOT NULL
    ) STORED,
...
CREATE INDEX IF NOT EXISTS idx_audit_sensitive ON audit_log(has_sensitive_data) WHERE has_sensitive_data;