...
var (
	...
	db2ReservedTopLevelWords = []string{
		"ADD",
		"AFTER",
		"ALTER COLUMN",
		"ALTER TABLE",
	...
	db2ReservedTopLevelWordsNoIndent = []string{"INTERSECT", "INTERSECT ALL", "MINUS", "UNION", "UNION ALL"}

	db2ReservedNewlineWords = []string{
		"AND",
		"CROSS JOIN",
		"INNER JOIN",
		"JOIN",
...
func NewDB2TokenizerConfig() *TokenizerConfig {
	return &TokenizerConfig{
		...
		ReservedNewlineWords:          db2ReservedNewlineWords,
		ReservedTopLevelWordsNoIndent: db2ReservedTopLevelWordsNoIndent,
		StringTypes:                   []string{`""`, "''", "``", "[]", "$$"},
		OpenParens:                    []string{"("},
		CloseParens:                   []string{")"},
...
func (ssf *DB2Formatter) Format(query string) string {
	return core.FormatQuery(
		...
		func(tok types.Token, previousReservedWord types.Token) types.Token {
			if tok.Type == types.TokenTypeReservedTopLevel && tok.Value == setKeyword && previousReservedWord.Value == "BY" {
